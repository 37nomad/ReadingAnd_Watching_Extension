(() => {
	...
	if (!window.__scraperInitialized__) {
		...
		class EnhancedContentScraper {
			constructor() {
				console.log(
					"Enhanced Content Scraper with YouTube transcript support initialized"
				);
			}
			...
			async extractContent() {
				...
				if (wordCount < minWords) {
					console.log(
						`Content too short (${wordCount} words), skipping...`
					);
					return null;
				}
			...
			extractTranscriptText() {
				...
				for (const selector of transcriptSelectors) {
					...
					if (elements.length > 0) {
						const transcriptParts = Array.from(elements)
							.map((el) => {
			...
			async extractTranscriptManually() {
				...
				console.log(
					'1. Look for "Show transcript" button and click it'
				);
				console.log(
					"2. Wait a moment, then run: scraper.extractTranscriptText()"
				);

				const transcriptElements = document.querySelectorAll(
					'[class*="transcript"], [aria-label*="transcript"]'
				);
				console.log(
					"Found transcript-related elements:",
		...
		const requiredActiveTime = 3 * 60 * 1000; 
		// -----For Quick Testing comment out the above code and use the below line------
		// const requiredActiveTime = 15 * 1000;
		...
		document.addEventListener("visibilitychange", () => {
			if (document.visibilityState === "visible") {
				...
				if (lastActiveTimestamp) {
					...
					console.log(
						`Tab inactive. Active time so far: ${Math.floor(
							activeTime / 1000
						)} sec`
					);
				}
		...
		setInterval(async () => {
			if (isTabActive && !hasExtracted) {
				...
				if (activeTime >= requiredActiveTime) {
					console.log(
						"Required active time reached! Extracting content..."
					);
					const content = await scraper.extractContent();
					if (content) {
						console.log(
							"Content extracted after active time:",
							content
						);
